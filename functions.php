...
function menuSteal($menuUrl){
    try {
        ...
        if(!$html){
            throw new Exception("Do not steal to menu!");
        }
    } catch (Exception $e) {
        echo $e->getMessage(); 
    }
...
function threadSteal($threadListUrl){
    try {
        $html = file_get_contents($threadListUrl);
        $encode = mb_detect_encoding($html);
        if($encode === "UTF-8"){
        ...
        if(!$html){
            throw new Exception("Do not steal to thread!");
        }
    } catch (Exception $e) {
        echo $e->getMessage(); 
    }
...
function resSteal($threadUrl){
    try {
        $resBaseHtml = file_get_contents($threadUrl);
        ...
        if(!$resBaseHtml){
            throw new Exception("Do not steal to res!");
        }
    } catch (Exception $e) {
        echo $e->getMessage(); 
    }
...
function resBodyImgLinkProc($dbHandle, $tempResBodyByResData, $threadSha, $regex, $prepareStForTUTI, $imgBaseUrl){

    $countOfResBodyByResData = count($tempResBodyByResData);
    for ($i=0; $i < $countOfResBodyByResData; $i++) {
        $tempResBodyByResData[$i]["res_rowbody"] = $tempResBodyByResData[$i]["res_body"];
        $isImg = preg_match_all($regex, $tempResBodyByResData[$i]["res_body"], $resImgLink, PREG_SET_ORDER);
        ...
        for ($k=0; $k < $countOfResImgLink; $k++) {
            ...
            array_push($resImgData, ["img_url"=>$imgReUrl, "img_link"=>$imgUrlForView, "img_name"=>$shaedImgFileName]);
        }
        if($resImgData){
